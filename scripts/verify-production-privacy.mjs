...
function noExportOrPersistence(value, code) {
  if (own(value, "persist") && value.persist !== false) fail(code);
...
function checkProjectedPrivacySettings(settings) {
  ...
  if (obs.logs !== undefined && obs.logs !== null) {
    ...
    falseRequired(obs.logs, "invocation_logs", "invocation_logs_not_explicitly_disabled");
    optionalSampling(obs.logs, "logs_sampling_invalid");
    noExportOrPersistence(obs.logs, "logs_export_or_persistence_not_disabled");
  }
  if (obs.traces !== undefined && obs.traces !== null) {
    ...
    noExportOrPersistence(obs.traces, "traces_export_or_persistence_not_disabled");
  ...
  return {
    ...
    invocation_logs_disabled: true,
    tracing_disabled: true,
    issues_disabled: true,
    no_persistence_overrides: true,
...
export function privacyFailureReport(error) {
  return { schema_version: 1, status: "failed", check: "production_privacy_settings", worker: WORKER,
