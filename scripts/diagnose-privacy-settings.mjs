...
for (const endpoint of ["script-settings","settings"]) {
 let response;
 try {
  response = await fetch("https://api.cloudflare.com/client/v4/accounts/"+account+"/workers/scripts/oneclick-chatgpt/"+endpoint, {method:"GET",redirect:"error",signal:AbortSignal.timeout(15000),headers:{Authorization:"Bearer "+token,Accept:"application/json"}});
