...
test("MCP initialises and lists tools", async () => {
  assert.equal((await call("initialize")).result.serverInfo.name, "one-click");
  const tools = (await call("tools/list")).result.tools;
  assert.equal(tools.length, 6);
  const fullHandoff = tools.find((tool) => tool.name === "oneclick_prepare_full_handoff");
  assert.equal(fullHandoff.annotations.readOnlyHint, true);
  assert.equal(fullHandoff.annotations.openWorldHint, true);
  assert.equal(fullHandoff.annotations.destructiveHint, false);
});
