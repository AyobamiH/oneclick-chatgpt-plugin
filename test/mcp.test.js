...
test("Unreleased authenticated tools are not exposed", async () => { const result = await call("tools/call", { name: "oneclick_prepare_full_handoff", arguments: {} }); assert.equal(result.result.isError, true); assert.equal(result.result.structuredContent.status, "blocked"); });
